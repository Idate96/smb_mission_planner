    ...
    def createStateMachine(self):
        ...
        with state_machine:
            smach.StateMachine.add(
                "Mission 1",
                DefaultMission(self.missions_data["test_mission"], self.topic_names),
                transitions={
                    "Completed": "Success",
                    "Aborted": "Failure",
...
class NavigateDetect(smach.State):
    def __init__(self, mission_data, topic_names):
        smach.State.__init__(
            self, outcomes=["Completed", "Aborted", "Next Waypoint", "New Artefact"]
        )
        self.mission_data = mission_data
        self.waypoint_idx = 0
        self.topic_names = topic_names
        ...
        self.artifact_subscriber = rospy.Subscriber(
            topic_names["W_artefact"], PointStamped, self.artifactPositionCallback
        )
...
class SpinningMission(smach.State):
    def __init__(self, mission_data, topic_names):
        smach.State.__init__(
            self, outcomes=["Completed", "Aborted", "Next Waypoint", "End Circle"]
        )
        self.mission_data = mission_data
        self.waypoint_idx = 0
        self.topic_names = topic_names
