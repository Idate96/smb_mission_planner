...
from geometry_msgm.msg import PointStamped
...
class MissionPlan():
    ...
    def createStateMachine(self):
        ...
        with state_machine:
            smach.StateMachine.add('Follow Waypoints', NavigateDetectMission(self.missions_data['test_mission'], self.topic_names),
            transitions={'Completed':'Success', 'Aborted':'Failure', 'Next Waypoint':'Follow Waypoints', 'New Artefact: Approach Artefact'})
            smach.StateMachine.add('Approach Artefact', NavigateDetectMission(self.missions_data['test_mission'], self.topic_names),
            transitions={'Completed':'Success', 'Approached':'Approach Artefact', 'Step Back: Approach Artefact','Line Spanned':'Follow Waypoints'})
...
class NavigateDetectMission(smach.State):
    """ There are 2 missions:
        1- Follow the waypoint: the robot has to move from one waypoint to the next one while listening to 
            odom('base_pose') and artefacts('w_artefact')
            STATES AND TRANSITIONS:
            Waypoint Reached or Time Elapsed -> Next WayPoint
            No More Waypoint or No wp at all -> Completed
            Unreachable WayPoint
    
        2- Approach Artefact: when a NEW artefact is detected the robot will visually servo it and move on the line 
    ...
    def __init__(self, mission_data, topic_names):
        ...
        self.waypoint_pose_publisher = rospy.Publisher(topic_names['waypoint'], PoseStamped, queue_size=1)
        self.base_pose_subscriber = rospy.Subscriber(topic_names['base_pose'], Odometry, self.basePoseCallback)
        # Artefact_mapping node will be published on the W_landmark topic in the odometry frame. 
        self.wart_position_subscriber = rospy.Subscriber(topic_names['W_artefact'], PointStamped, self.WlandPositionCallback)
        while self.waypoint_pose_publisher.get_num_connections() == 0 and not rospy.is_shutdown():
            rospy.loginfo_once("Waiting for subscriber to connect to '" +
                          topic_names['waypoint'] + "'.")
            rospy.sleep(1)
        while self.base_pose_subscriber.get_num_connections() == 0 and not rospy.is_shutdown():
            rospy.loginfo_once("Waiting for publisher to connect to '" +
                          topic_names['base_pose'] + "'.")
            rospy.sleep(1)
        while self.wart_position_subscriber.get_num_connections() == 0 and not rospy.is_shutdown():
            rospy.loginfo_once("Waiting for publisher to connect to '" +
